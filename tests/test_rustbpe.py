...
def test_correctness(enwik8_small) -> None:
    ...
    assert fast_reference_ids == slow_reference_ids, (
    ...
    assert custom_match(hf_ids, fast_reference_ids), (
...
def test_interface(enwik8_small) -> None:
    ...
    assert all(x == ids for x in ids_new), (
...
def test_batch_encode_performance(enwik8_large):
    ...
    for i, (seq, batch) in enumerate(
        zip(sequential_results, batch_results, strict=False)
    ):
    ...
    if speedup < 1.5:
        warnings.warn(
            f"batch_encode() speedup was only {speedup:.2f}x (expected >1.5x)",
            stacklevel=2,
        )
